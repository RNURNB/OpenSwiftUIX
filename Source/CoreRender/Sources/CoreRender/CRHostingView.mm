...
- (instancetype)initWithHierarchy:(CRNodeHierarchy*) hierarchy
                         context:(CRContext *)context
                    withOptions:(CRNodeLayoutOptions)options {
  if (self = [super initWithFrame:CGRectZero]) {
    if (@available(iOS 13, *)) {
      self.backgroundColor = [UIColor systemBackgroundColor];
...
-(void) setNeedsBuild {
  [_body buildHierarchyInView:self constrainedToSize:self.bounds.size withOptions:_options];
}
